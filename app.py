...
with st.sidebar:
    ...
    st.divider()
    st.caption("🔐 Reads OpenAI key from env var **MY_NEW_APP_KEY** (set in Render → Environment).")
...
if run:
    ...
    with tab_files:
        if not files:
            ...
            for f in files:
                ...
                # crude guess for code highlighting
                lang = "java" if suffix in ["java", "xml"] and "dockerfile" not in path.lower() else \
                       "python" if suffix in ["py"] else \
                       "csharp" if suffix in ["cs"] else \
                       "javascript" if suffix in ["js", "mjs", "cjs"] else \
                       "yaml" if suffix in ["yml", "yaml"] or "dockerfile" in path.lower() else \
                       "json" if suffix in ["json"] else \
                       "md" if suffix in ["md"] else "text"
    ...
    with tab_usage:
        st.write("Token usage reported by the API (if available).")
